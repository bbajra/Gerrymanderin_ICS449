...
public class BoundaryCreator {
	...
	//This holds the orignal File in lines of string
	private ArrayList<String> unModed = new ArrayList<String>();
	//This hold all the lat and longitudes
	private ArrayList<LongLatHolder> longLatHolderList = new ArrayList<LongLatHolder>();
	private ArrayList<Line> lineList = new ArrayList<Line>();
	private ArrayList<Line> LineListFinal = new ArrayList<Line>();
	public BoundaryCreator(ArrayList<String> x){
		this.unModed = x;
		putIntoArray();
		parseIntoLines();
	}
...
/**
 * This takes the geojson file and breaks it into the long and latitude and put its into
 * longLatHolderList
 */
	//no , after ] then break
...
//4136
	...
	/**
	 * run this after putIntoArray is done
	 * This takes the long & lat and turns into lines
	 */
	public void parseIntoLines(){

		for(int i = 0; i<longLatHolderList.size();i++){
			if(i+2<=longLatHolderList.size()){

				LongLatHolder longLat1 = longLatHolderList.get(i);
				LongLatHolder longLat2 = longLatHolderList.get(i+1);
				if((longLat1.getCongDist()==longLat2.getCongDist())&&(longLat1.getCountyID()==longLat2.getCountyID())&&
						(longLat1.getLegDist()==longLat2.getLegDist())&&(longLat1.getSenateDist()==longLat2.getSenateDist())){
					Line line = new Line();
					line.setPoint1(longLat1);
					line.setPoint2(longLat2);
					lineList.add(line);	
				}
			}
		}
	...
	/**
	 * X is what kind of regional boundaries you want to draw
	 * This method choose what kind of map you want and gets rid of the uesslesses lines
	 * ex:
	 * If a line in County 1 is = line in County 2 keep the line, and get rid of the rest
	 * @param x
	 * if int = 1 go by county ID
	 * if int =2 go by congDist
	 * if int = 3 go by Senatedist
	 * if int  = 4 go by legdist
	 */
	public void deleteUseLess(int x){
		String type;
		if(x == 1){
			type = "getCountyID()";
		}else if(x==2){
		...
		for(int i = 0;i<lineList.size()-1;i++){
			...
			for(int j = 1;j<lineList.size();j++){
				...
				if(line1.getPoint1().getCountyID()!=line2.getPoint1().getCountyID()){
					...
					if(line1.getPoint1().getX()==line2.getPoint1().getX()&&
							...
							line1.getPoint1().getCountyID()!=line2.getPoint1().getCountyID()
							){
						LineListFinal.add(line1);
					}
				}
			}
		}
	...
	public ArrayList<String> getUnModed() {
		return unModed;
	}
	public ArrayList<Line> getLineListFinal() {
		return LineListFinal;
	}
	public ArrayList<LongLatHolder> getLongLatHolderList() {
		return longLatHolderList;
	}
	public ArrayList<Line> getLineList() {
		return lineList;
	}
	/**
	 * This sets parameters for the longLatholder objects
	 * @param obj
	 * @param x
	 * @return
	 */
		public LongLatHolder Setparameters(LongLatHolder obj, String x){
			//8 cong dist
