...
public class PrecintCreator {
	...
	private ArrayList<String> untouchedList;//This is the code as string
	private ArrayList<Precints> precintList = new ArrayList<Precints>();//All created precints will be held in here
	...
	public PrecintCreator(ArrayList<String> geoJsonList){
		...
		start();
		getAdjacentsAsString();
		addAdjacentsToPrecints();
		getIndex();
	}
	...
	public void getIndex(){
		for(int i = 0;i<precintList.size();i++){
			precintList.get(i).setPlaceInIndex(i);
		}
	}
	...
	/***
	 * this method will add the adjacent precints to the precint object
	 */
	private void addAdjacentsToPrecints(){
		AdjacentPrecint adjacentPrecint;
		for(int i = 0;i<precintList.size();i++){
			Precints precint = precintList.get(i);
			for(int j = 0;j<precint.adjacentListAsString.size();j++){
				String precintNameAsString = precint.adjacentListAsString.get(j);
				for(int k = 0;k<precintList.size();k++){
					Precints precint1 =  precintList.get(k);
					String precintName = precint1.getPrecintName();
					if(precintNameAsString.equals(precintName)){
						precint.adjacentList.add(precint1);
						adjacentPrecint = new AdjacentPrecint(precint.getPrecintName(),precint.getPlaceInIndex());
						precint.adjacentPrecintList.add(adjacentPrecint);
						break;
					}
				}
	...
	/**
	 * This gets the adjacent precints into a string
	 * @param fullGeoJsonCode
	 * @return
	 */
	private void getAdjacentsAsString(){
		for(int j = 0;j<precintList.size();j++){
			Precints precint = precintList.get(j);
			String fullGeoJsonCode = precint.getGeoJsonCode();
			int indexToStart = getPropertyInt(fullGeoJsonCode, "AdjacentPrecints");
			char[] org = fullGeoJsonCode.toCharArray();
			String adjacentPreint = "";
			for(int i = indexToStart;i<org.length;i++){
				if(org[i]=='"'){//this is the ending point
					precint.AddToAdjacentListAsString(adjacentPreint);
					break;
				}
				else if(org[i]==','){
					precint.AddToAdjacentListAsString(adjacentPreint);
					...
					adjacentPreint = adjacentPreint + org[i];
				}
			}
		}
	}
